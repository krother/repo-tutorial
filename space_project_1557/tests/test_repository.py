...
def test_create_and_get_location(example_location):
    repo_used.create_location("xyz123", example_location)
    loc = repo_used.get_location("xyz123", "Magrathea")
    assert loc.name == "Magrathea"


def test_create_game():
    game = repo_used.create_game(TEST_JSON, "Magrathea")
...
def test_create_and_get():
    game = repo_used.create_game(TEST_JSON, "Magrathea")
    game = repo_used.get_game(game.game_id)
    assert game.location.name == "Magrathea"


def test_create_multiple(location_batch):
    for loc in location_batch:
        repo_used.create_location("xyz", loc)
    for loc in location_batch:
        assert repo_used.get_location("xyz", loc.name)


def test_get_location_for_game(example_location):
    repo_used.create_location("xyz", example_location)
    game = repo_used.create_game(TEST_JSON, start_location="Magrathea")
    loc = repo_used.get_location(game.game_id, "Magrathea")
    assert loc.name == "Magrathea"
