...
def start_game(start_location="Pandalor") -> GameData:
    ...
    game = repo_used.create_game(galaxy_file=fn, start_location=start_location)
...
def execute_command(game_id: str, command: str) -> GameData:
    ...
    game = repo_used.get_game(game_id)
    for cmd in game.get_commands():
        if cmd.name == command:
            cmd.callback()
            break
    repo_used.update_game(game)
    repo_used.update_location(game.game_id, game.location)
    return _get_game_data(game)
